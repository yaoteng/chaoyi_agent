...
> 配套《潮驿项目重新定位方案》《潮驿Agent项目竞品分析》。本作战书沿用此前直播方向的**渠道与节奏方法论**，但 hook 与素材全部替换为「社区 agent 化」。

## 0. 战略支点

**一句话定位（hook）**：
> 「让每一个小社群，都能拥有自己的数字社工。」
> 一套 MIT 开源、跨宿主（Claude Code / WorkBuddy / Cursor）、即插即用的**社群智能体技能库**。

**为什么能冲星**：
- Agent/Skill 是 2026 最大确定性赛道，但「社区 agent 化」是蓝海（竞品分析已确认：无统治级开源项目）。
- 我们首发就是**可运行、可复制的 5 个 Skills + 模板 + 索引**，不是 PPT。
- 复用已验证打法：你方 FMCG 分销 SaaS 冲 1000 star、掘金过审长文、四端工程经验。
- 中文优先 + 标准兼容（Skills 1.0 / MCP 0.4）→ 同时吃中文社群与海外 agent 生态两波流量。
