...
> ✅ **定位确认（2026-09-06）**：chaoyi（潮驿/炒意）是**直播社区类品牌**，本项目即其 **agent 化工具箱**，定位为「**直播社区运营 agent 化**」。`posts/01~10`、`demo-script.md` 与本文正文均已改写为直播社区口径（参考 01 与《决策确认与执行记录》第六节）。**节奏 / Gate / KPI 框架不变**，可直接按 D-1 → D0~D6 执行。
...
| D0 | 中文技术圈 | 掘金长文 + 开源中国 + 知乎 | 120–220 | 120–220 |
| D1 | 海外 HN/Reddit | Show HN + Reddit(r/selfhosted,r/opensource,r/Twitch) + Dev.to | 180–320 | 300–540 |
...
- [x] 录制 30 秒 demo（食光小馆直播社区样例跑通 5 个直播社区 Skill，见 `demo-script.md`），导出 `launch/demo.mp4` + `launch/demo_cover.png` 封面。
...
2. **开源中国（OSChina）**：发 `posts/02_oschina.md`——先提交到「开源项目」库（oschina.net/project）拿展示位，再同步发一篇博客长文。
...
2. **Reddit**：`r/selfhosted` + `r/opensource` + `r/Twitch` 各发 `posts/05_reddit.md`，遵守各版规则（勿跨版重复刷）。
3. **Dev.to**：发 `posts/06_devto.md`（可同步 Medium）。
4. 关键：在 HN/Reddit 评论里**真诚回答技术细节**，不 defend、不刷。

### D2 · X / 公众号
1. **X（推特）**：发 `posts/07_x_thread.md` 长线程（5–8 条），带 `#OpenSource #AIagents #Livestreaming` 标签，@ 相关开源大佬（别 spam）。
2. **微信公众号**：发 `posts/08_wechat.md`（可配 demo 动图）。
3. 把 D0/D1 的高赞评论截图，作为"社区反馈"发 X 置顶。

### D3 · 视频
1. 发 B站 `posts/demo-script.md` 对应的 30 秒视频，标题带关键词"开源 Agent / 直播社区智能体"。
2. 视频简介 + 评论区置顶仓库链接。
...
## 5. 合规红线（贯穿全程）
- 开源代码分发**不需要**直播类牌照；但**不得以开源为名实际运营无牌公开直播服务**（本项目是运营工具箱，已彻底归档直播代码，无此风险）。
- Skill 内容不得含以下能力（见 `CONTRIBUTING.md` 与 `skills/README.md`）：
  - 绕过直播 / 平台合规（未成年人保护、打赏限额、版权曲库、内容安全审核）；
  - 伪造、篡改直播 / 观众数据（观看 / 互动 / 转化 / 礼物）；
...
## 6. 发布后必做（工程侧）
- 开 `Discussions` 作为"直播社区运营 agent 化"问答区。
- 建 `good first issue` 若干（翻译某 Skill、补一个垂类直播社区 Skill）。
